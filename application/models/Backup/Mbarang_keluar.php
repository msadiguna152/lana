...
class Mbarang_keluar extends CI_Model {
	public function insert()
	{
		...
		for ($i = 0; $i < $id_barang; $i++) {
			...
			$cek_barang = $this->db->select('*')->from('stok_puskesmas')->where('id_barang',$this->input->post('id_barang['.$i.']'))->where('id_puskesmas',$this->input->post('id_puskesmas'))->get();

			if ($cek_barang->num_rows()==0 AND $this->input->post('keluar_karena')=="Permintaan PKM") {
				$data_barang['id_puskesmas'] = $this->input->post('id_puskesmas');
				$data_barang['id_barang'] = $this->input->post('id_barang['.$i.']');
				$data_barang['ed_barang_puskesmas'] = $this->input->post('ed_barang_keluar['.$i.']');
				$this->db->insert('stok_puskesmas', $data_barang);
			} else {
				if ($this->input->post('keluar_karena')=="Permintaan PKM") {
	...
	public function get($dari_tanggal,$sampai_tanggal)
	{
		if ($dari_tanggal!=NULL AND $sampai_tanggal!=NULL) {
			...
			->where('tanggal_barang_keluar>=',$dari_tanggal)
			->where('tanggal_barang_keluar<=',$sampai_tanggal)
			...
			->join('puskesmas','puskesmas.id_puskesmas=barang_keluar.id_puskesmas','LEFT')
			->join('sumber','sumber.id_sumber=barang_keluar.id_sumber','LEFT')
			->where('MONTH(tanggal_barang_keluar)',$filter['1'])
			->where('YEAR(tanggal_barang_keluar)',$filter['0'])
			->order_by('id_barang_keluar','DESC')->get();
		}
	...
	{
		return $this->db->query("SELECT * from barang_keluar JOIN puskesmas ON puskesmas.id_puskesmas=barang_keluar.id_puskesmas JOIN sumber ON sumber.id_sumber=barang_keluar.id_sumber where barang_keluar.id_barang_keluar='$id'")->row_array();
	}
